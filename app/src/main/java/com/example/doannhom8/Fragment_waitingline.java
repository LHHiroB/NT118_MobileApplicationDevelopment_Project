...
public class Fragment_waitingline extends Fragment {
    ...
    private void LoadQueue() {
        db.collection("/FoodQueue/").orderBy("TIME", Query.Direction.ASCENDING).get().addOnCompleteListener(task1 -> {
            ...
            for (DocumentSnapshot data : task1.getResult()) {
                ...
                data.getDocumentReference("food_name").get().addOnCompleteListener(task2 -> {
                    task2.getResult().getReference().getParent().getParent().get().addOnCompleteListener(task5 -> {
